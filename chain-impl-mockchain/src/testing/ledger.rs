...
use crate::{
    ...
    stake::PoolsState,
    testing::{
        builders::{BftBlockBuilder, GenesisOptimumBlockBuilder},
...
impl ConfigBuilder {
    ...
    pub fn build(self) -> ConfigParams {
        ...
        ie.push(ConfigParam::ConsensusGenesisOptimumActiveSlotsCoeff(
...
impl TestLedger {
    ...
    pub fn forge_empty_block(&self, stake_pool: &StakePool) -> Block {
        self.forge_optimum_block_with_fragments(stake_pool, Vec::new())
    }

    pub fn apply_empty_optimum_block(&mut self, stake_pool: &StakePool) -> Result<(), Error> {
        self.apply_optimum_block(stake_pool, vec![])
    }
    ...
    pub fn apply_optimum_block(
        &mut self,
        stake_pool: &StakePool,
        fragments: Vec<Fragment>,
    ) -> Result<(), Error> {
        let block = self.forge_optimum_block_with_fragments(stake_pool, fragments);
        self.apply_block(block)
    }

    pub fn forge_optimum_block_with_fragments(
        &self,
        stake_pool: &StakePool,
        fragments: Vec<Fragment>,
    ) -> Block {
        GenesisOptimumBlockBuilder::new()
            .with_date(self.date())
    ...
    pub fn fire_leadership_event(
        ...
        for stake_pool in stake_pools {
            if selection
                ...
                .is_some()
            {
                self.apply_optimum_block(&stake_pool, fragments)?;
                return Ok(true);
            }
        }
        self.forward_date();
