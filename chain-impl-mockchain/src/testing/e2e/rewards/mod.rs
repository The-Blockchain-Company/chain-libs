...
pub fn rewards_empty_pots() {
    ...
    assert!(ledger.apply_empty_optimum_block(&stake_pool).is_ok());
    ledger.distribute_rewards().unwrap();
...
pub fn rewards_from_fees() {
    ...
    assert!(ledger
        .apply_optimum_block(&stake_pool, vec![fragment])
        .is_ok());
...
pub fn rewards_owner_of_many_stake_pool() {
    ...
    let fragment = fragment_factory.transaction(&bob, &clarice, &mut ledger, 100);
    assert!(ledger
        .apply_optimum_block(&first_alice_stake_pool, vec![fragment])
        .is_ok());

    let fragment = fragment_factory.transaction(&clarice, &bob, &mut ledger, 100);
    assert!(ledger
        .apply_optimum_block(&second_alice_stake_pool, vec![fragment])
        .is_ok());
...
pub fn rewards_delegators_of_many_stake_pool() {
    ...
    assert!(ledger
        .apply_optimum_block(&alice_stake_pool, vec![fragment])
        .is_ok());

    let fragment = fragment_factory.transaction(&clarice, &david, &mut ledger, 100);
    assert!(ledger
        .apply_optimum_block(&bob_stake_pool, vec![fragment])
        .is_ok());
