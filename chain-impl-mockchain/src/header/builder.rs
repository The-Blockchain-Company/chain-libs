...
use super::cstruct;
use super::header::{HeaderBft, HeaderGenesisOptimum, HeaderUnsigned};
...
/// Header Builder (finalize Genesis-Optimum part)
pub struct HeaderGenesisOptimumBuilder<GpBuildingState: ?Sized>(
    cstruct::Header,
...
impl HeaderBuilder<HeaderCommonDone> {
    ...
    /// Tentatively transition to a Genesis-Optimum Header builder
    pub fn into_genesis_optimum_builder(
        self,
    ) -> Option<HeaderGenesisOptimumBuilder<HeaderSetConsensusData>> {
        match self.0.version() {
            cstruct::VERSION_GP => Some(HeaderGenesisOptimumBuilder(self.0, PhantomData)),
            _ => None,
        }
    }
}
...
impl HeaderGenesisOptimumBuilder<HeaderSetConsensusData> {
    pub fn set_consensus_data(
        self,
        node_id: &PoolId,
        vrf_proof: &VrfProof,
    ) -> HeaderGenesisOptimumBuilder<HeaderSetConsensusSignature> {
        let mut hdr = self.0;
        hdr.set_gp_node_id(node_id.into());
        hdr.set_gp_vrf_proof(&vrf_proof.0);
        HeaderGenesisOptimumBuilder(hdr, PhantomData)
    }
}
...
impl HeaderGenesisOptimumBuilder<HeaderSetConsensusSignature> {
    pub fn get_authenticated_data(&self) -> &[u8] {
        self.0.as_slice().slice_gp_auth()
    }

    /// Set the signature in the Genesis-Optimum header and return a finalized Genesis-Optimum Header
    pub fn set_signature(self, kes_signature: KesSignature) -> HeaderGenesisOptimum {
        let mut hdr = self.0;
        hdr.set_gp_kes_signature_slice(kes_signature.0.as_ref());
        HeaderGenesisOptimum(hdr)
    }

    /// Just a helper to set the signature directly from what the secret key generate
    pub fn sign_using(self, kes_signing_key: &SecretKey<SumEd25519_12>) -> HeaderGenesisOptimum {
...
mod tests {
    ...
    #[test]
    #[should_panic]
    pub fn wrong_version_genesis_optimum() {
        HeaderBuilderNew::new(BlockVersion::Ed25519Signed, &Contents::empty())
            .set_parent(&TestGen::parent_id(), chain_length())
            .set_date(block_date())
            .into_genesis_optimum_builder()
