...
use super::cstruct;
use super::deconstruct::{BftProof, Common, GenesisOptimumProof, Proof};
...
/// Finalized Genesis-Optimum Header
#[derive(Clone, PartialEq, Eq)]
pub struct HeaderGenesisOptimum(pub(super) cstruct::Header);
...
pub enum Header {
    ...
    GenesisOptimum(HeaderGenesisOptimum),
    Bft(HeaderBft),
}
...
impl HeaderGenesisOptimum {
    pub fn id(&self) -> HeaderId {
        HeaderId::hash_bytes(self.0.as_slice().as_slice())
    }

    pub fn generalize(self) -> Header {
        Header::GenesisOptimum(self)
    }
}
...
impl Header {
    ...
    fn get_cstruct(&self) -> cstruct::HeaderSlice<'_> {
        match self {
            ...
            Header::GenesisOptimum(h) => h.0.as_slice(),
    ...
    pub fn as_auth_slice(&self) -> &[u8] {
        match self {
            ...
            Header::GenesisOptimum(_) => self.get_cstruct().slice_gp_auth(),
        }
    }
    ...
    pub fn block_version(&self) -> BlockVersion {
        match self {
            ...
            Header::GenesisOptimum(_) => BlockVersion::KesVrfproof,
    ...
    pub fn from_slice(slice: &[u8]) -> Result<Header, HeaderError> {
        ...
        match BlockVersion::from_u16(hdr.version()).expect("header slice only know version") {
            ...
            BlockVersion::KesVrfproof => Ok(Header::GenesisOptimum(HeaderGenesisOptimum(hdr))),
        }
    }
    ...
    pub fn proof(&self) -> Proof {
        match self.block_version() {
            ...
            BlockVersion::KesVrfproof => Proof::GenesisOptimum(GenesisOptimumProof {
