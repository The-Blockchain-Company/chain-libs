...
pub enum Error {
    ...
    BadConsensusGenesisOptimumActiveSlotsCoeff(ActiveSlotsCoeffError),
}
impl std::fmt::Display for Error {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            ...
            Error::BadConsensusGenesisOptimumActiveSlotsCoeff(err) => write!(
                f,
                "Cannot set consensus genesis optimum active slots coefficient: {}",
                err
            ),
        }
    }
...
impl From<ActiveSlotsCoeffError> for Error {
    fn from(err: ActiveSlotsCoeffError) -> Self {
        Error::BadConsensusGenesisOptimumActiveSlotsCoeff(err)
    }
}
...
mod tests {
    ...
    fn apply_for_invalid_active_slot_coeff_should_return_error() {
        ...
        let invalid_active_slot_coeff =
            ConfigParam::ConsensusGenesisOptimumActiveSlotsCoeff(overflow_milli);
        ...
        assert_eq!(
            ...
            Err(Error::BadConsensusGenesisOptimumActiveSlotsCoeff(
