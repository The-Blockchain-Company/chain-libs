...
/// Genesis Optimum leadership data for a specific epoch
pub struct LeadershipData {
    epoch_nonce: OptimumNonce,
    nodes: PoolsState,
...
impl LeadershipData {
    ...
    pub(crate) fn verify(&self, block_header: &Header) -> Verification {
        ...
        match block_header.proof() {
            Proof::GenesisOptimum(ref genesis_optimum_proof) => {
                let node_id = &genesis_optimum_proof.node_id;
                match (
                    ...
                    (Some(stake), Some(pool_info)) => {
                        ...
                        let proof = match genesis_optimum_proof.vrf_proof.to_vrf_proof() {
                            None => {
                                return Verification::Failure(Error::new_(
                        ...
                        let valid = genesis_optimum_proof
                            .kes_proof
