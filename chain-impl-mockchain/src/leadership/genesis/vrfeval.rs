...
use crate::chaineval::OptimumNonce;
...
impl Input {
    ...
    fn create(epoch_nonce: &OptimumNonce, slotid: SlotId) -> Self {
...
pub struct VrfEvaluator<'a> {
    ...
    pub nonce: &'a OptimumNonce,
    pub slot_id: SlotId,
    pub active_slots_coeff: ActiveSlotsCoeff,
}

pub(crate) fn witness_to_nonce(witness: &Witness) -> OptimumNonce {
...
impl<'a> VrfEvaluator<'a> {
    ...
    pub fn verify(
        ...
        witness: &'a Witness,
    ) -> Result<OptimumNonce, VrfEvalFailure> {
        let input = Input::create(self.nonce, self.slot_id);
...
fn get_nonce(os: &WitnessOutput) -> OptimumNonce {
    let mut nonce = [0u8; 32];
    let out = os.to_output(&[], DOMAIN_NONCE);
    nonce.copy_from_slice(out.as_ref());
    OptimumNonce::from_output_array(nonce)
}
