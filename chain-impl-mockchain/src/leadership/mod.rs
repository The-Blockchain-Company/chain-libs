...
pub enum LeaderOutput {
    None,
    Bft(BftLeaderId),
    GenesisOptimum(PoolId, genesis::Witness),
}

pub enum LeadershipConsensus {
    Bft(bft::LeadershipData),
    GenesisOptimum(genesis::LeadershipData),
}
...
impl LeadershipConsensus {
    ...
    fn verify_version(&self, block_version: BlockVersion) -> Verification {
        match self {
            ...
            LeadershipConsensus::GenesisOptimum(_) if block_version == BlockVersion::KesVrfproof => {
                Verification::Success
            }
    ...
    fn verify_leader(&self, block_header: &Header) -> Verification {
        match self {
            ...
            LeadershipConsensus::GenesisOptimum(genesis_optimum) => genesis_optimum.verify(block_header),
        }
    }
    ...
    fn is_leader(&self, leader: &Leader, date: BlockDate) -> LeaderOutput {
        match self {
            ...
            LeadershipConsensus::GenesisOptimum(genesis_optimum) => match leader.genesis_leader {
                None => LeaderOutput::None,
                Some(ref gen_leader) => {
                    match genesis_optimum.leader(&gen_leader.node_id, &gen_leader.vrf_key, date) {
                        Ok(Some(witness)) => {
                            LeaderOutput::GenesisOptimum(gen_leader.node_id.clone(), witness)
                        }
                        _ => LeaderOutput::None,
                    }
                }
...
impl Leadership {
    pub fn new(epoch: Epoch, ledger: &Ledger) -> Self {
        let inner = match ledger.settings.consensus_version {
            ...
            ConsensusType::GenesisOptimum => {
                LeadershipConsensus::GenesisOptimum(genesis::LeadershipData::new(epoch, ledger))
            }
        };
        Leadership {
            epoch,
    ...
    pub fn stake_distribution(&self) -> Option<&StakeDistribution> {
        match &self.inner {
            ...
            LeadershipConsensus::GenesisOptimum(inner) => Some(inner.distribution()),
        }
    }
...
mod tests {
    ...
    #[test]
    fn consensus_verify_version_for_optimum() {
        let ledger = TestGen::ledger();

        let data = genesis::LeadershipData::new(0, &ledger);

        let gen_leadership_consensus = LeadershipConsensus::GenesisOptimum(data);
